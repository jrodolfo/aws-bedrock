...
next_image_name() {
  ...
  for file in "$OUTPUT_DIR"/${IMAGE_PREFIX}[0-9][0-9][0-9][0-9]${IMAGE_SUFFIX}(N); do
    num="${file:t}"
    num="${num#${IMAGE_PREFIX}}"
...
request_file="$(mktemp "/tmp/bedrock-request.XXXXXX.json")"
...
printf '%s' "$image_b64" | base64 --decode > "$output_image"
